...
import { str_dclivechat } from "./strings.js";
import { decode, debug, randomBlanks, getNow, createElement } from "./common.js";
...
let savePackage = (packageData) => {
    let savedPackages = getOption('dccon-pk') ?? {};
    if (savedPackages[packageData.idx] != undefined) {
        let existing = savedPackages[packageData.idx];
        if (existing.buy || !packageData.buy) return;
        existing.detail = packageData.detail;
        existing.buy = true;
    } else {
        savedPackages[packageData.idx] = packageData;
    }
...
let getDccon = (packageTitle, dcconTitle) => {
    let packageData = getPackage(packageTitle);
    if (!packageData || !packageData.detail) return null;
    for (let detail of packageData.detail) {
...
let createPackagePage = (packageData) => {
    let packageIdx = packageData.idx;
    let packageTitle = packageData.title;
    ...
    if (existing) {
        ...
        else createElement('img', link, { src: getDcconUrlFromCode(packageData.code) });
    }
    ...
    for (let dccon of packageData.detail) {
        let code = dccon.code;
...
onApplyFunc['dccon-pk'] = (packages) => {
    for (let title in packages) {
        ...
        let packageData = packages[title];
        for (let detail of packageData.detail) savedDccons[detail.code] = detail;
        savedPackageTitles.push(packageData.title);
    }
};
...
let footerOption = createElement('a', optionsWrite, {
    [onclick]: () => {
        ...
        let footerStatic = createElement(spanString, null, {
            [innerText]: '- ' + str_dclivechat,
            [onclick]: () => footerInput.focus()
        }, 'footer-static');
        footerInput.insertAdjacentElement('afterend', footerStatic);
    }
}, 'opt.r');
